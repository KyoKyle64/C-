...
int main()
{
 ...
 cout << "Please enter the amount of months for your statement: ";
 cin >> numMonth;
 //Loop - Get user data from account owner & Calculate monthly balance
 for (month = 1; month <= numMonth; month++)
 {
  cout << "\nMonth: " << month;
  ...
  {
    cout << "\n\nYou have overdrawn your account. You can no longer withdraw at this time. \n\n";
    return 0;
  }

  intrestEarned = (intrestRate / 12)*((deposit/2)+(totalBalance-deposit)+(withdrawl/2));
  totalIntrest = totalIntrest + intrestEarned;
  totalBalance = totalBalance + intrestEarned;
 ...
 cout << "Total withdrawals:  - $""     " << totalWithdrawls <<endl;
